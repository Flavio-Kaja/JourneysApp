...
public interface ICurrentUserService : IJourneyServiceScopedService
{
...
public sealed class CurrentUserService : ICurrentUserService
{
    ...
    public int? DailyGoal
    {
        get
        {
            var dailyGoalClaim = _httpContextAccessor.HttpContext
                ?.User
                ?.Claims
                ?.FirstOrDefault(x => x.Type == "DailyGoal")
                ?.Value;

            if (dailyGoalClaim != null && int.TryParse(dailyGoalClaim, out int dailyGoal))
            {
                return dailyGoal;
            }
