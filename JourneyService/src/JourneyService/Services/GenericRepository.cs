...
public interface IGenericRepository<TEntity> : IJourneyServiceScopedService
    ...
    where TEntity : BaseEntity
{
    ...
    protected GenericRepository(JourneysDbContext dbContext)
    {
    ...
    public virtual async Task<TEntity> GetById(Guid id, bool withTracking = true, CancellationToken cancellationToken = default)
    {
        ...
        return entity ?? throw new NotFoundException($"{typeof(TEntity).Name} with an id '{id}' was not found.");
    }
